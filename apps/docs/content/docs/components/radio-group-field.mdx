...
| `slotProps` | `Partial<BaseFieldSlotProps> & Partial<{ option: Partial<RadioOptionSlotProps> }>` | | — | Extra props passed to each slot. Field-level replaces provider-level per key. |
...
interface RadioOption<T = string> {
  value: T
  label: string
  description?: string   // Optional sub-label below the option label
