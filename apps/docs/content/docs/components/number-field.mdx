...
<NumberField name="price" label="Price" type="number" />
```
...
function CustomLabel({ htmlFor, children, required }: LabelSlotProps) {
  return (
    <label htmlFor={htmlFor} className="text-sm font-semibold text-purple-600">
      {children}
